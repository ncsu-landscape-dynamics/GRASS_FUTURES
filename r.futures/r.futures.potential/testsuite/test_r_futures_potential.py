...
class TestPotential(TestCase):
    ...
    def test_potential_run_dredge(self):
        ...
        with open(self.output, 'r') as f:
            ...
            for line in f.readlines():
                ...
                self.assertEqual(len(line), 6)
                if i == 0:
                    self.assertEqual(sorted(line), ['ID', 'Intercept', 'devpressure', 'lakes_dist_km', 'slope', 'streets_dist_km'])
                i += 1
            self.assertEqual(i, 14)
    ...
    def test_potential_run_dredge_export(self):
        ...
        self.assertModule('r.futures.potential', flags='d', input='sampling',
                          columns=['devpressure', 'lakes_dist_km', 'slope', 'streets_dist_km'],
                          developed_column='urban_change', subregions_column='zipcodes',
                          min_variables=3, max_variables=4, nprocs=2, fixed_columns=['slope'],
        ...
        with open(self.output, 'r') as f:
            ...
            for line in f.readlines():
                ...
                if i == 0:
                    self.assertEqual(line, ['ID', 'Intercept', 'devpressure', 'streets_dist_km', 'slope'])
                i += 1
            self.assertEqual(i, 14)
        with open(self.dredge_out, 'r') as f:
            ...
            for line in f.readlines():
                ...
                if i == 0:
                    self.assertEqual(line, ["ID", "Intercept",
                                            "devpressure", "lakes_dist_km", "slope",
                                            "streets_dist_km", "df", "logLik", "AIC", "delta", "weight"])
                i += 1
